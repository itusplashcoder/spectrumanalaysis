...
def top_10_keywords_by_country(spark):
    ...
    tokenized_df = fact_news_df.withColumn("word", explode(split(lower(col("news_text")), "\\W+")))
    stop_words = StopWordsRemover.loadDefaultStopWords("english")
    stop_words_df = spark.createDataFrame([(word,) for word in stop_words], ["stop_word"])
    filtered_df = tokenized_df.join(stop_words_df, tokenized_df.word == stop_words_df.stop_word, "left_anti")

    # Group by country and word, then count occurrences
    keyword_freq_df = filtered_df.groupBy("country_id", "word").agg(count("word").alias("frequency"))
    ...
    top_keywords_df.write.mode("overwrite").format("delta").saveAsTable("trending_keywords_by_country")
...
def top_10_keywords_by_category(spark):
    ...
    keyword_freq_df = filtered_df.groupBy("category_id", "word").agg(count("word").alias("frequency"))
    keyword_freq_df = keyword_freq_df.filter(keyword_freq_df.word != "")
    ...
    top_keywords_by_category_df.write.mode("overwrite").format("delta").saveAsTable("trending_keywords_by_category")
